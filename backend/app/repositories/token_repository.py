...
class TokenRepository(BaseRepository[RefreshToken]):
    ...
    async def get_verification_token(self, token: UUID) -> Optional[VerificationToken]:
        result = await self.db.execute(select(VerificationToken).where(VerificationToken.token == token))
        return result.scalar_one_or_none()

    async def create_verification_token(self, obj_in: dict) -> VerificationToken:
    ...
    async def delete_verification_tokens_for_user(self, user_id: UUID):
        await self.db.execute(delete(VerificationToken).where(VerificationToken.user_id == user_id))
        await self.db.flush()
