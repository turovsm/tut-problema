...
async def enrich_report_data(
    report: Report, db_session, current_user_id=None, vote_repo=None
):
...
class ReportService:
    def __init__(
        self, report_repo: ReportRepository, vote_repo: VoteRepository = None
    ):
        self.report_repo = report_repo
        self.vote_repo = vote_repo

    async def _enrich_reports_bulk(
        self, reports: list[Report], current_user_id=None
    ):
        if not reports:
            return reports
        ...
        if current_user_id and self.vote_repo:
            ...
            votes_map = await self.vote_repo.get_user_votes_for_reports(
                current_user_id, report_ids
            )

            for r in reports:
                if r.id in votes_map:
                    r.current_user_vote = (
                        "confirm" if votes_map[r.id].is_confirm else "dismiss"
                    )
    ...
    async def get_report_by_id(self, report_id: UUID, current_user_id=None):
        ...
        if report:
            return await enrich_report_data(
                report, self.report_repo.db, current_user_id, self.vote_repo
            )
        return None
    ...
    async def get_reports_paginated(
        ...
        query = select(Report).options(
            selectinload(Report.created_by), selectinload(Report.photos)
        )
        if issue_type:
        ...
        total = await self.report_repo.db.execute(
        ...
        enriched_reports = await self._enrich_reports_bulk(
            reports, current_user_id
        )
    ...
    async def get_nearby_reports(
        ...
        query = (
            ...
            .where(
                func.ST_DWithin(
                    Report.location, func.ST_GeomFromText(point, 4326), radius
                )
            )
            .order_by(
                func.ST_Distance(
                    Report.location, func.ST_GeomFromText(point, 4326)
                )
            )
            .limit(limit)
        )
    ...
    async def get_user_reports(self, user_id: UUID, limit: int, offset: int):
        ...
        reports = items.scalars().all()
        enriched_reports = await self._enrich_reports_bulk(
            reports, current_user_id=user_id
        )
        return enriched_reports, total.scalar()
    ...
    async def create_report(
        ...
        distance = calculate_distance_haversine(
            user_lat, user_lng, loc_lat, loc_lng
        )
        if distance > settings.MAX_REPORT_DISTANCE_METERS:
            raise ValueError(
                f"Distance {distance:.0f}m exceeds max allowed ({settings.MAX_REPORT_DISTANCE_METERS}m)."
            )
    ...
    async def update_report(
        ...
        if report.created_by_id != user_id and user_role not in [
            "moderator",
            "gov_org",
        ]:
