...
export class AuthService {
  ...
  me(): Observable<ApiResponseSuccess<User>> {
    return this.http
      .get<ApiResponseSuccess<User>>(`${environment.apiUrl}/api/users/me`, {
        withCredentials: true,
      })
      .pipe(tap((response) => this.currentUserSignal.set(response.data)));
  }
  ...
  verifyEmail(token: string): Observable<ApiResponseSuccess<null>> {
    return this.http.post<ApiResponseSuccess<null>>(
  ...
  changePassword(
    ...
    newPassword: string,
  ): Observable<ApiResponseSuccess<null>> {
    return this.http.post<ApiResponseSuccess<null>>(
